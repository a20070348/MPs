...
rho = 1000.0;g = 9.8; costheta1 = 0; costheta2 = 1.0/3.0;n1=100;n2=10;
beta1 =1:1:n1; beta2 =1:1:n2;
...
while (i<=n1)
    while (abs(Res(i)-Resf(i))>0.1)
        ...
        Res(i) = (wshsRe(i)*beta1(i))*D1^3/(2.0*nu);
    end
    i=i+1;
end
wshsRe = wshsRe * D1^2;
...
Resp = Dsp.^3*rho*g/(18*mu*nu);Respf = 0.0*beta2;wssp = 0.0*beta2;
i=1;
while (i<=n2)
    while (abs(Resp(i)-Respf(i))/Resp(i)>0.05)
        Respf(i) = Resp(i);  
        wssp(i) = Dsp(i)^2*rho*g/(18*mu)/(1+0.15*Resp(i)^0.687);
        Res(i) = wssp(i)*Dsp(i)/nu;
    end
    i=i+1;
end
wsspd = wssp;
